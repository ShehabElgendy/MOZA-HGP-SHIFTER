...
public class ShifterTest :MonoBehaviour
{
    ...
    void MozaShifterTest()
    {
        ...
        {
            Debug.Log(($"No MOZA Shifter device found, error = {error}"));
            return;
        }
        ...
        if (!device.Open())
        {
            Debug.Log("Device open failed.");
            return;
        }

        Debug.Log($"MOZA Shifter device '{device.Path}' is opened.");
        ...
        while (device.IsConnected)
        {
            ...
            Debug.Log($"The gear has been switched from {gear} to {currentGear}");
            gear = currentGear;
        }

        Debug.Log("The device has been disconnected.");
    }
}
